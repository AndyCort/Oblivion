...
export default function NavBar() {
  ...
  const resetSlider = () => {
    const activeItem = navMenuRef.current?.querySelector(
      "li a.active",
    ) as HTMLElement;
    if (activeItem) {
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (searchInputRef.current?.value.trim()) {
      navigate(
        `/search?s=${encodeURIComponent(searchInputRef.current.value.trim())}`,
      );
  ...
  return (
    <>
      <NavBarContainer $atTop={atTop} $navHidden={navHidden}>
        ...
        <NavRight $atTop={atTop} $mobileOpen={mobileOpen} data-card="glass">
      ...
      {mobileOpen && <MobileOverlay onClick={closeMobileMenu} />}
    </>
  );
}
...
const NavPill = styled.div<{ $atTop?: boolean }>`
  ...
  border: ${(props) =>
    props.$atTop ? "1px solid transparent" : "var(--border)"};
...
const NavLink = styled.a<{ $isActive?: boolean }>`
  ...
  padding: 8px 16px;
  color: ${(props) =>
    props.$isActive ? "var(--main-color)" : "var(--text-1)"};
  text-decoration: none;
